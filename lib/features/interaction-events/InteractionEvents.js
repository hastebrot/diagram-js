...
export default function InteractionEvents(eventBus, elementRegistry, styles) {
  ...
  function createHitStyle(element) {

    var traits = [ 'no-fill', 'no-border' ];

    if (isFrameElement(element)) {
      traits.push('stroke-events');
    }

    return styles.cls('djs-hit', traits, {
      stroke: 'white',
      strokeWidth: 15
    });
  ...
  eventBus.on([ 'shape.added', 'connection.added' ], function(event) {
    ...
    svgAttr(hit, createHitStyle(element));
